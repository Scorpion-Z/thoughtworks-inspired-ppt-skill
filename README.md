...
- no-crop full-slide presentation mode, keyboard/touch navigation, overview, and low-power mode.

## Highlights

- **Agent-first workflow**: designed for Codex, Claude Code, Cursor, and similar coding agents.
- **Browser-native deck**: outputs a self-contained HTML presentation directory that keeps the full 16:9 slide visible by default.
...
6. **Presenter controls stay minimal.** Generated decks keep the full slide visible; page rail and external counters stay hidden by default, while the keyboard hint auto-hides.
...
The bottom page rail and external counter are hidden by default, and the control hint auto-hides so the deck can present without persistent overlays. In overview mode, thumbnails are generated by the runtime from the real slide DOM; clicking a thumbnail jumps to that slide.
...
`contain` is the default mode: the full 16:9 slide remains visible in full-screen and non-16:9 browser windows. This may create letterboxing, but it prevents hidden content. Use `data-deck-fit="cover"` only when the user explicitly accepts cropping to fill the browser. The bottom page rail and external counter are hidden by default; press `Esc` for full-page navigation.
...
- full-slide `contain` scaling with no viewport cropping;
...
- desktop full-screen, mobile, and tablet viewport smoke checks;
- Chrome-like desktop viewport overlap and no-crop checks.
